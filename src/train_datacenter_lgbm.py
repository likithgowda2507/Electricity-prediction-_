"""
Train Datacenter-tuned LightGBM models (Energy + Bill) on the building dataset.
Uses deeper hyperparameters (more leaves, more depth) for complex patterns.
...
def main():
    print("=" * 60)
    print("  DC-LightGBM Model Training (Building Dataset)")
    print("=" * 60)

    X_train, X_test, y_train, y_test, feature_names, le = load_and_prepare()

    # Deeper hyperparameters for complex pattern capture
    params = {
        'n_estimators': 800,
        'max_depth': 10,
        'learning_rate': 0.03,
    ...
    # ---- Energy Model ----
    print("\n--- Training Energy Model (DC-LightGBM) ---")
    ...
    # ---- Bill Model ----
    print("\n--- Training Bill Model (DC-LightGBM) ---")
    ...
    # ---- Save ----
    joblib.dump(energy_model, "energy_model_dc_lgbm.pkl")
    joblib.dump(bill_model, "bill_model_dc_lgbm.pkl")
    print("\n[OK] Models saved: energy_model_dc_lgbm.pkl, bill_model_dc_lgbm.pkl")
    ...
    with open("feature_importances_dc_lgbm.json", "w") as f:
        json.dump(importance_data, f, indent=2)
    print("[OK] Feature importances saved: feature_importances_dc_lgbm.json")
